...
namespace HCB.GridSystem
{
    public abstract class PlaceableBase : SelectableBase, IPlaceable
    {
        ...
        public void SetFreeLastPlacedTiles()
        {
        ...
        protected List<ITile> GetNeighborTiles(ITile pivotTile)
        {
            ...
            {
                int x = pivotTile.Grid.x + GridPoints[i].x;
                int y = pivotTile.Grid.y + GridPoints[i].y;
        ...
        private bool CanPlace()
        {
            ...
            if (PlaceableManager.Instance.PlaceBlockers.Count != 0)
                return false;

            bool canPlace = true;
            for (int i = 0; i < GridPoints.Count; i++)
            {
                int x = LastInteractedPivotTile.Grid.x + GridPoints[i].x;
                int y = LastInteractedPivotTile.Grid.y + GridPoints[i].y;

                ITile tile = TileManager.Instance.GetTile(x, y);
                if (tile == null || !tile.IsPurchased || (tile.IsOccupied && tile.PlacedItem != Placeable))
                {
                    canPlace = false;
                    break;
                }
